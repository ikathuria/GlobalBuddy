  ...
  return (
    <>
      <section className="gb-card">
        <h2>Survival plan & cultural bridge</h2>
        <p style={{ margin: "0 0 1rem", color: "var(--gb-muted)", fontSize: "0.9rem" }}>
          The 30-day plan is structured JSON from your evidence bundle only — no invented entities. Explanations adapt
          US terms to your home context.
        </p>
        ...
        {plan && !loadingPlan && (
          <div>
            ...
            <div className="gb-steps">
              {(plan.steps || []).map((s, i) => (
                <div key={i} className="gb-step">
                  <div>
                    <strong>{s.day_range}</strong>
                  </div>
                  <div>{s.action}</div>
                  <div className="gb-step-detail">
                    {(s.entities || []).join(" · ")} — {s.dependency_reason}
                  </div>
                </div>
              ))}
            </div>
            {(plan.warnings || []).length > 0 && (
              <div style={{ marginTop: "1rem" }}>
                <h3 style={{ fontSize: "0.85rem", color: "var(--gb-muted)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Warnings</h3>
        ...
        <p style={{ margin: "0 0 0.85rem", color: "var(--gb-muted)", fontSize: "0.88rem" }}>Opens the cultural bridge drawer with a structured explanation.</p>
        <div className="gb-actions">
          <input
            value={term}
          ...
          />
          <button type="button" className="gb-btn gb-btn-secondary" onClick={() => explainTerm()} disabled={loadingBridge || loadingPlan}>
            Explain term
          </button>
        </div>
