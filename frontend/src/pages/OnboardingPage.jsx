...
export default function OnboardingPage() {
  ...
  const nodeIndex = useMemo(
    () => new Map((match?.subgraph?.nodes || []).map((n) => [n.id, n])),
    [match?.subgraph?.nodes],
  );

  const scrollToStep = useCallback(
    (step) => stepRefs[step]?.current?.scrollIntoView({ behavior: "smooth", block: "start" }),
    [stepRefs],
  );
  ...
  const openStep = useCallback(
    (step) => {
      if (step > 1 && !hasProfile) {
        setActiveStep(1);
        setBanner({ type: "error", message: "Start with your profile first. We will unlock your plan and graph right after." });
        scrollToStep(1);
        return;
      }
      if (step === 2 && returningToUs) { setActiveStep(3); scrollToStep(3); return; }
      setActiveStep(step);
      scrollToStep(step);
    },
  ...
  const onNodeSelect = useCallback((node) => setSelectedNode(node), []);
  ...
  const handleProfileMatch = (data) => {
    ...
    requestAnimationFrame(() => scrollToStep(hasLivedInUsBefore ? 3 : 2));
  };
  ...
  const heroPrimaryCta = () => openStep(hasProfile ? (returningToUs ? 3 : 2) : 1);
  const heroSecondaryCta = () => openStep(hasProfile ? 3 : 1);

  const stepState = (step) => {
    if (step === 1) return hasProfile ? "done" : activeStep === 1 ? "active" : "pending";
    if (step === 2) {
      if (!hasProfile) return "locked";
      if (returningToUs || hasPlan) return "done";
  ...
  const stepSummary = (step) => {
    if (step === 1) return "Share basic context so we can tailor support to your background and destination.";
    if (step === 2) return returningToUs ? "Skipped for returning US users so you can move straight to exploration." : "Get a calm, week-by-week action plan with culturally relevant context.";
    return "Use your support graph to find mentors, housing leads, events, and practical next steps.";
  };

  const isStepOpen = (step) => (!hasProfile && step > 1 ? false : activeStep === step);
  const isStepLocked = (step) => step > 1 && !hasProfile;

  const profileName = match?.evidence_bundle?.student_profile?.full_name || user?.full_name || "You";
    ...
    <div className="gb-app">
      <nav className="gb-nav" aria-label="Primary">
        ...
        <div className="gb-nav-right">
          <StatusPanel compact />
          {user ? (
            <button type="button" className="gb-btn gb-btn-ghost" onClick={() => navigate("/dashboard")}>
              Dashboard
            </button>
          ) : (
            <button type="button" className="gb-btn gb-btn-ghost" onClick={() => navigate("/auth")}>
              Log in
            </button>
          )}
        </div>
      ...
      <div className="gb-main">
        <header className="gb-hero gb-card">
          ...
          <div className="gb-hero-ctas">
            <button type="button" className="gb-btn gb-btn-primary" onClick={heroPrimaryCta}>{primaryLabel}</button>
            <button type="button" className="gb-btn gb-btn-secondary" onClick={heroSecondaryCta}>{secondaryLabel}</button>
          </div>
        </header>
        ...
        <section className="gb-journey-shell" aria-label="Guided onboarding steps">
          ...
          <article ref={stepRefs[2]} className="gb-card gb-journey-card" id="step-plan">
            <div className="gb-journey-head">
              <button type="button" className={stepClass(2)} onClick={() => openStep(2)} disabled={isStepLocked(2)} aria-expanded={isStepOpen(2)}>
          ...
          <article ref={stepRefs[3]} className="gb-card gb-journey-card" id="step-graph">
            <div className="gb-journey-head">
              <button type="button" className={stepClass(3)} onClick={() => openStep(3)} disabled={isStepLocked(3)} aria-expanded={isStepOpen(3)}>
